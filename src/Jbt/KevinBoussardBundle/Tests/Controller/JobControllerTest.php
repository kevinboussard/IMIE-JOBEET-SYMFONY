...
namespace Jbt\KevinBoussardBundle\Tests\Controller;
...
class JobControllerTest extends WebTestCase
{
    public function getMostRecentProgrammingJob()
    {
        ...
        $query = $em->createQuery('SELECT j from JbtKevinBoussardBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
    ...
    public function getExpiredJob()
    {
        ...
        $query = $em->createQuery('SELECT j from JbtKevinBoussardBundle:Job j WHERE j.expires_at < :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        return $query->getSingleResult();
    ...
    public function testIndex()
    {
        ...
        $this->assertEquals('Jbt\KevinBoussardBundle\Controller\JobController::indexAction', $client->getRequest()->attributes->get('_controller'));
        ...
        $this->assertEquals('Jbt\KevinBoussardBundle\Controller\JobController::showAction', $client->getRequest()->attributes->get('_controller'));
    ...
    public function testJobForm()
    {
        ...
        $this->assertEquals('Jbt\KevinBoussardBundle\Controller\JobController::newAction', $client->getRequest()->attributes->get('_controller'));

        $form = $crawler->selectButton('Preview your job')->form(array(
            'job[company]'      => 'Sensio Labs',
            'job[url]'          => 'http://www.sensio.com/',
            'job[file]'         => __DIR__.'/../../../../../web/bundles/jbtkevinboussard/images/sensio-labs.gif',
        ...
        $this->assertEquals('Jbt\KevinBoussardBundle\Controller\JobController::previewAction', $client->getRequest()->attributes->get('_controller'));
    }

    public function testErrorJobForm()
    {
        $client = static::createClient();
    ...
    public function testInsertBdd()
    {
        $kernel = static::createKernel();
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');

        $query = $em->createQuery('SELECT count(j.id) from JbtKevinBoussardBundle:Job j WHERE j.location = :location AND j.is_activated IS NULL AND j.is_public = 0');
    ...
    public function testPublishJob()
    {
        ...
        $query = $em->createQuery('SELECT count(j.id) from JbtKevinBoussardBundle:Job j WHERE j.position = :position AND j.is_activated = 1');
    ...
    public function getJobByPosition($position)
    {
        ...
        $query = $em->createQuery('SELECT j from JbtKevinBoussardBundle:Job j WHERE j.position = :position');
    ...
    public function testExtendJob()
    {
        ...
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        $job = $em->getRepository('JbtKevinBoussardBundle:Job')->findOneByPosition('FOO5');
