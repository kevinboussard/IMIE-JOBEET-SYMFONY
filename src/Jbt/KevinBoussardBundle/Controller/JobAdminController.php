...
class JobAdminController extends Controller
{
    public function batchActionExtend(ProxyQueryInterface $selectedModelQuery)
    {
        ...
        try {
            ...
            $this->get('session')->set('sonata_flash_error', $e->getMessage());
    ...
    public function batchActionDeleteNeverActivated()
    {
        ...
        $nb = $em->getRepository('JbtKevinBoussardBundle:Job')->cleanup(60);

        if ($nb) {
            $this->get('session')->set('sonata_flash_success',  sprintf('%d never activated jobs have been deleted successfully.', $nb));
        } else {
            $this->get('session')->set('sonata_flash_info',  'No job to delete.');
        }
